...
// Ollama connection (default: http://127.0.0.1:11434)
const ollama = new Ollama();

class AIInterviewEngine {
  
  async generateQuestions(cvData, field) {
    // Model ka naam yahan set karein (make sure ye 'ollama pull' kiya ho)
    const MODEL_NAME = "llama3"; 

    console.log(`🧠 AI Engine (Ollama): Generating VERBAL questions for '${field}' using ${MODEL_NAME}...`);
    ...
    try {
      // 👇 OpenAI call ki jagah ab Ollama call hai
      const response = await ollama.chat({
        ...
        format: "json", // ✨ Ye bohot zaroori hai taakay Ollama sirf JSON return kare
        stream: false   // Hum poora jawab aik sath chahte hain
      });

      // Ollama ka response structure thora alag hota hai (response.message.content)
      const content = JSON.parse(response.message.content);
      
      console.log("✅ Questions Generated by Ollama");
      return content.questions; 

    } catch (error) {
      console.error("❌ Ollama Generation Failed:", error);
      
      // Fallback: Agar Ollama fail ho jaye (ya model load na ho)
      return [
        { 
          id: 1, 
